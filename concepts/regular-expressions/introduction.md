...
Regular expressions (regex) are a powerful tool for working with strings in Clojure. Because Clojure is hosted on the JVM, the regular expressions are those used in Java. String patterns representing the regular expression's meaning are first compiled then used for matching all or part of a string.

You can construct a regex in Clojure using a literal syntax, which is a string with a hash sign in front:

``` clojure
#"test"
```

In Clojure, the most common way to create regular expressions is using `re-pattern`. To match a _string literal_, we can use the string itself as a pattern.
